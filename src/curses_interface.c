...
FILE *_t3_putp_file; /**< @c FILE struct corresponding to the terminal. Used for tputs in
                        ::_t3_putp. */
...
int _t3_setupterm(const char *term, int fd) {
  char term_buffer[COPY_BUFFER_SIZE];
  char *term_ptr = NULL;
  int error;
  ...
  if (term != NULL) {
    /* Copy the term name into a new buffer, because setupterm expects a char *
       not a const char *. */
    strncpy(term_buffer, term, COPY_BUFFER_SIZE);
    term_buffer[COPY_BUFFER_SIZE - 1] = 0;
